...
describe("Form Field", () => {
  describe("as TextField", () => {
    it('renders as an input[type="text"]', () => {
      const { getByLabelText } = render(FormField, { type: FORM_FIELD_TYPE, ...PROPS })
    ...
    it('fires a change event when something has been typed', async () => {
      ...
      const { component, getByLabelText } = render(FormField, { type: FORM_FIELD_TYPE, ...PROPS })
      ...
      expect(mockComponentEvent).toHaveBeenLastCalledWith({ ...PROPS, value: "a value that I've typed", name: PROPS.id, })
