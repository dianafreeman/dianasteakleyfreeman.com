...
describe("Form Field", () => {
  describe("as TextAreaField", () => {
    it('renders as an textbox', () => {
      const { getByRole } = render(FormField, { type: FORM_FIELD_TYPE, ...TEST_PROPS })
    ...
    it('fires a change event when something has been typed', async () => {
      const { component, getByRole } = render(FormField, { type: FORM_FIELD_TYPE, ...TEST_PROPS })
      const input = getByRole('textbox')
      ...
      await user.type(input, "a longer value, maybe even a paragraph, that I've typed")

      expect(mockComponentEvent).toHaveBeenCalled()
      expect(mockComponentEvent).toHaveBeenLastCalledWith({ ...TEST_PROPS, value: "a longer value, maybe even a paragraph, that I've typed", name: TEST_PROPS.id, })
