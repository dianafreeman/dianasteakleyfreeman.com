...
const ProjectPageTemplate = ({data}) => {
  ...
  return (
    <Layout parallax={false}>
      ...
      <ArticleWrapper>
        <Img fluid={project.featured_media.localFile.childImageSharp.fluid} style={{width: '300px', marginRight: '1em', float: 'left'}}/>
        <article>
          <div dangerouslySetInnerHTML={{ __html: project.content }} style={{display: 'block'}}/> 
        </article>
        <aside>
          <p>This app uses...{ technologies.map( tech => icondata.filter()`${tech.name}, `) }</p>
        </aside>
      </ArticleWrapper>
    </Layout>
  )
