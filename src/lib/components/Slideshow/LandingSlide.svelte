<script>
  ...
  $: isLongWord = slide.title?.length > 8;
  onMount(() => {
    ...
    <div class="flex flex-col xl:flex-row flex-1 align-middle justify-center h-full px-16">
      <div bind:clientWidth={inlineSize} class="flex flex-col xl:m-auto xl:w-1/2">
        <h1
          ...
          class="font-bold xl:text-center text-7xl lg:text-8xl xl:text-9xl"
        >
          {slide.title}
        </h1>
      </div>
      <div class="flex flex-col xl:m-auto">
        <h2 class="text-transparent-60 font-manrope font-black text-5xl xl:text-6xl py-3">
          {displayNumber}
        </h2>
        <p class="font-manrope text-xl xl:text-2xl ">{slide.description}</p>
      </div>
    </div>
  </div>
  <div />
